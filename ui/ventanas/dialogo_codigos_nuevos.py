...
no están mapeados en ``codigos_conceptos.json``, ``foapal.json`` ni en
``codigos_ignorados.json``. El usuario puede:

- Ignorar los códigos (se guardan en ``codigos_ignorados.json`` con su descripción).
...
class DecisionCodigos:
    ...
    # código -> descripción del concepto
    ignorar: dict[str, str] = field(default_factory=dict)
...
class DialogoCodigosNuevos(QDialog):
    ...
    def _guardar(self) -> None:
        ...
        for i in range(self._tabla.rowCount()):
            ...
            descripcion = self._tabla.item(i, 1).text()
            combo = self._tabla.cellWidget(i, 2)
            if combo.currentText() == ACCION_IGNORAR:
                decision.ignorar[codigo] = descripcion
                continue
